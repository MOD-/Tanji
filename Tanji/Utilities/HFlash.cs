...
{
    // TODO: Redo all of this, someday.
    public class HFlash : ShockwaveFlash
    {
        private readonly Dictionary<ASClass, ushort> _headerCache;
        private readonly Dictionary<ASClass, bool> _isOutgoingCache;
        private readonly Dictionary<ASClass, string> _classHashCache;
        private readonly Dictionary<string, List<ASClass>> _hashClassCache;
        private readonly Dictionary<ASInstance, ASClass> _incomingParsersCache;
        ...
        public HFlash(byte[] data)
            : base(data)
        {
            _headerCache = new Dictionary<ASClass, ushort>();
            _isOutgoingCache = new Dictionary<ASClass, bool>();
            _classHashCache = new Dictionary<ASClass, string>();
            _hashClassCache = new Dictionary<string, List<ASClass>>();
        ...
        public bool FindMessageInstances()
        {
                ...
                habboMessages.Constructor.Body.Bytecode.ToArray()))
            {
                while (mapReader.Position != mapReader.Length)
                {
                    ...
                    ASClass messageClass = abc.FindClassByName(messageType.ObjName);

                    if (isOutgoing)
                    {
                        OutgoingTypes[header] = messageClass;
                        if (header == 4000) ExtractBuild(messageClass.Instance);
                    }
                    else if (isIncoming) IncomingTypes[header] = messageClass;

                    _headerCache[messageClass] = header;
                    _isOutgoingCache[messageClass] = isOutgoing;
                }
            }
        ...
        protected void ExtractBuild(ASInstance instance)
        {
            ASMethod method = instance.FindMethod("*", "Array").Method;
            ...
            using (var inCode = new FlashReader(method.Body.Bytecode))
            {
                while (inCode.Position != inCode.Length)
                {
                    ...
                    if (op == OPCode.PushString)
                    {
                        var stringIndex = (int)values[0];

                        string value = instance.ABC
                            .Constants.Strings[stringIndex];
        ...
        public bool IsOutgoing(ASClass asClass)
        {
            if (_isOutgoingCache.ContainsKey(asClass))
                return _isOutgoingCache[asClass];

            return false;
        }
        public ushort GetHeader(ASClass asClass)
        {
            if (_headerCache.ContainsKey(asClass))
                return _headerCache[asClass];

            return ushort.MaxValue;
        }
        public List<ASClass> GetClasses(string hash)
        {
            if (_hashClassCache.ContainsKey(hash))
                return _hashClassCache[hash];
        ...
        {
            if (_classHashCache.ContainsKey(asClass))
                return _classHashCache[asClass];
            ...
            using (var hashInput = new BinaryWriter(hashStream))
            {
                ...
                string hash = GetHash(hashStream.ToArray());
                _classHashCache[asClass] = hash;

                if (!_hashClassCache.ContainsKey(hash))
                    _hashClassCache[hash] = new List<ASClass>();

                if (!_hashClassCache[hash].Contains(asClass))
                    _hashClassCache[hash].Add(asClass);
