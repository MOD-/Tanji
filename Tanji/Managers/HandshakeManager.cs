...
namespace Tanji.Managers
{
    public class HandshakeManager
    {
        ...
        {
            int exponent = REAL_EXPONENT;
            if (!int.TryParse(MainUI.Game.Exponent, out exponent))
                exponent = REAL_EXPONENT;

            string modulus = (string.IsNullOrWhiteSpace(MainUI.Game.Modulus) ?
                REAL_MODULUS : MainUI.Game.Modulus);

            Remote.Exchange = new HKeyExchange(exponent, modulus);
            Local.Exchange = new HKeyExchange(FAKE_EXPONENT, FAKE_MODULUS, FAKE_PRIVATE_EXPONENT);
        }
        private void FinalizeHandshake()
        {
            //Local.Encrypter = new Rc4(_localSharedKey);
            MainUI.Connection.DataOutgoing -= DataOutgoing;
            MainUI.Connection.DataIncoming -= DataIncoming;
        }
        ...
        private void ReplaceRemotePublicKey(InterceptedEventArgs e)
        {
            ...
            // DO NOT assign Local.Encrypter yet, otherwise this
            // packet will get encrypted, and the client will not
            // be able to decrypt it to generate the shared key.

            //Remote.Decrypter = new Rc4(_remoteSharedKey);
