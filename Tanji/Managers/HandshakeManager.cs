...
namespace Tanji.Managers
{
    public class HandshakeManager
    {
        ...
        public const int DEFAULT_REAL_EXPONENT = 65537;
            ...
            try
            {
                switch (e.Step - _inStepOffset)
                {
                    case 1:
                    {
                        ...
                        {
                            remoteE = int.Parse(
                                MainUI.Game.Exponent, NumberStyles.HexNumber);
                        }

                        string remoteN = DEFAULT_REAL_MODULUS;
                        if (!string.IsNullOrWhiteSpace(MainUI.Game.Modulus))
                        {
                            remoteN = MainUI.Game.Modulus;
                        }

                        Remote.Exchange = new HKeyExchange(remoteE, remoteN);
